...
import { IBetDocument, IBetsByDay, ICurrentMatch, IErrorMatches, IHitsBet, IMatchDay, IUserAndState, MatchDay } from "../types/types"
...
export const GetCurrentDays = (today: Date): MatchDay[] => {
    ...
    for (let i = -2; i <= 2; i++) {
        ...
        MATCH_DAYS.push({
            ...
            dateFullShort: `${currentDate.getFullYear()}-${formattedNumber(currentDate.getMonth() + 1)}-${formattedNumber(currentDate.getDate())}`,
...
export const GetMatchesOfTheDay = async () => {
    ...
    const weekendMatches = results.concat(thursdayMatches, fridayMatches, saturdayMatches, sundayMatches, mondayMatches)
...
export const TimeRemainig = (dateFirstMatch: string) => {
    const dateMatch = (new Date(dateFirstMatch).getTime()) - ((1000 * 60 * 60) * 1)
    const today = new Date().getTime()
    const timeOut = dateMatch - today
    const remainingHours = Math.floor((((timeOut / 1000) / 60) / 60))
    const remainingMinutes = Math.floor(((((timeOut / 1000) / 60) / 60) - remainingHours) * 60)
    const remainingSeconds = Math.floor((timeOut / 1000) - ((remainingHours * 60 * 60)) - (remainingMinutes * 60))

    const timeRemainig = `${remainingHours.toString().padStart(2, "0")}:${remainingMinutes.toString().padStart(2, "0")}:${remainingSeconds.toString().padStart(2, "0")}`
...
export const SortByHits = (order: "asc" | "des", betsArray: IBetDocument[], results: string[]): IHitsBet[] => {
