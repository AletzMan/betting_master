...
export function CardUser({ user, index, setUsersData }: Props) {
    ...
    return (
        <article className={`${styles.users_user} ${viewDetails === index && styles.users_userActive}`} key={user.id} onClick={(e) => HandleViewDetails(e, index)}>
            ...
            <div className={styles.users_options}>
                <div className={`flex items-center justify-center w-7 h-7 rounded-sm ${user.notifications ? "bg-(--background-info-color)" : "bg-transparent"}`} ><i className={`${styles.users_iconNoti} ${user.notifications && styles.users_iconNotiActive} ${user.notifications ? "pi pi-bell" : "pi pi-bell-slash"}`} /></div>
