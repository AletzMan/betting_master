...
export default function HeaderTable({ hiddenNames, setHiddenNames, matchDayInfo, totalBets }: Props) {
    ...
    return (
        <>
            {matchDayInfo &&
                <div className="sticky top-1 flex flex-col h-24 w-full bg-(--surface-e) border-b-1 border-(--surface-c) mt-1 rounded-sm z-5 overflow-hidden">
                    <div className="flex flex-col items-center justify-between w-full">
                        ...
                        <div className="flex flex-row gap-0.5">
                            ...
                            <button className="flex items-center justify-center w-11 h-11 bg-(--surface-b) rounded-md border-1 border-(--surface-d)" onClick={handleOrder}>
                                {!orderAsc ? <i className="pi pi-sort-amount-down"></i> : <i className="pi pi-sort-amount-up"></i>}
                            </button>
                        </div>
                    </div>
                    <button className="absolute right-0 top-[5px] flex items-center justify-center h-10 w-6 rounded-l-md  bg-(--primary-color) text-(--surface-b)" onClick={HandleSetVisibilityNames}>
