    ...
    */
    ...
    return (
        ...
        /*<main className={`${styles.main} ${isLandscape && styles.main_landscape}`}> 
            ...
            {(!matches.isFinishGame || viewBets) && <>
                ...
                {!loading && !myBets?.isNotBetsPaid && myBets.hasBets && bets && bets.length > 0 && matches?.matches[0]?.status !== "Sin comenzar" && (
                    <>
                       {matchDayInfo && matchDayInfo!.results?.length > 0 &&
                    <>
                        <section className={`relative grid w-full gap-1 pr-1 max-w-max border-1 border-transparent scrollbarXY  bg-(--surface-b) rounded-md transition-all ease-in-out delay-150 ${hiddenNames ? "grid-cols-[2.8em_1fr]" : "grid-cols-[13em_1fr]"}  `}>
            ...
            {matches.isFinishGame && !viewBets &&
                 {bets && <WinningBets bets={bets} matches={matchDayInfo} />}
            }
        </main>*/
    )
}