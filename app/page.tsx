...
import { TeamsLogosNews, Teams as TemasNames } from "./constants/constants"
...
export default async function Home() {
	...
	return (
		<main className="flex flex-col items-center justify-start pt-6 pb-4  h-svh">
			<Teams />
			<article className="relative flex flex-col items-center justify-start pt-7 gap-3.5 w-full h-[calc(100svh-21em)] p-1">
...
const header = (title: string) => {
	return (
		...
		<div className="grid grid-cols-[2.5em_3fr] gap-2.5 items-center justify-start px-2 py-2 bg-(--surface-b) border-b-1 border-b-(--surface-c)">
			{React.isValidElement(TeamsLogosNews.find(team => team.name === findFirstMatch(title, TemasNames))?.logo) ? (
				React.cloneElement(TeamsLogosNews.find(team => team.name === findFirstMatch(title, TemasNames))?.logo as ReactSVGElement, { className: 'w-7 h-7' })
			) : (
			...
			{React.isValidElement(TeamsLogosNews.find(team => team.name === findFirstMatch(title, TemasNames))?.logo) ?
				<span className="text-(--text-color) font-semibold" >{findFirstMatch(title, TemasNames)}</span>
